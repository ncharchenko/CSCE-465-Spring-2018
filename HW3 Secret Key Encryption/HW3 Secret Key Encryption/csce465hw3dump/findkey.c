...
0x13,0xe1,0x0d,0x1d,0xf4,0xa2,0xef,0x2a,0xd4,0x54,0x0f,0xae,0x1c,0xa0,0xaa,0xf9}; // Given ciphertext

unsigned char *plaintext = "This is a top secret."; // Given plaintext
...
/* Perform the encryption */
    ...
    OPENSSL_assert(EVP_CIPHER_CTX_key_length(&ctx) == 16); // Checking key length-from example link
    OPENSSL_assert(EVP_CIPHER_CTX_iv_length(&ctx) == 16); // Checking IV length-from example link
    /*Set key and IV, then begin encrypting! */
    ...
    if(fp != NULL){
        while( fgets (line, 255, fp) != NULL ) {
            ...
            if ( 0 == check_if_is_key(line) ){
                printf("I found a key and it is %s\n", line);
                break;
            }
        }
        fclose(fp);
